...
decoder_printable:= 'FileRemaker(encoded_string) {`n' .
                    '    b:=Buffer(1),  f:=FileOpen(A_Temp "\file", "w")`n' .
                    '    For ch in StrSplit(encoded_string)`n' .
                    '        NumPut("UChar", Mod(Ord(ch)-33,256), b), f.RawWrite(b)`n' .
                    '    f.Close()`n' .
...
FileRemaker(encoded_string) {
    b:=Buffer(1),  f:=FileOpen(A_Temp "\file", "w")
    For ch in StrSplit(encoded_string)
        NumPut("UChar", Mod(Ord(ch)-33,256), b), f.RawWrite(b)
    f.Close()
    Return(A_Temp "\file")
}
